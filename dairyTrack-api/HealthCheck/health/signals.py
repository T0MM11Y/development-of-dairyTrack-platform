...
def check_and_update_health_status(sender, instance, created, **kwargs):
    ...
    # 🔥 Buat notifikasi hanya saat pemeriksaan baru dan abnormal
    # 🔥 Buat notifikasi hanya saat pemeriksaan baru dan abnormal
    if abnormal and created:
        user = instance.checked_by  # Ambil langsung user yang login
        if user:
...
def check_reproduction_alert(sender, instance, created, **kwargs):
    if created:
        alerts = instance.is_alert_needed()  # Harus return list[str]
        user = instance.created_by  # 🔄 Ambil user yang membuat data
        if user:
