...
const Sidebar = ({ isCollapsed, isMobile, isOpen, onClose }) => {
  ...
  const theme = useMemo(
    () => ({
      ...
      lightBorder: isDarkMode ? "#444" : "#eee",
      buttonBg: isDarkMode ? "#444" : "#f5f5f5",
      buttonText: isDarkMode ? "#fff" : "#333",
      activeItemBg: isDarkMode ? "#2d2d3c" : "#e6f0ff",
      hoverBg: isDarkMode ? "#2a2a3a" : "#f0f0f0",
      submenuActiveBg: isDarkMode ? "#2a4436" : "#e6fff2",
      submenuActiveText: isDarkMode ? "#00e676" : "#198754",
      submenuText: isDarkMode ? "#ccc" : "#555",
      expandedMenuBg: isDarkMode ? "#2a2a3a" : "#f8f8f8",
    }),
    [isDarkMode]
  );
  ...
  const MenuItem = ({ menu }) => (
    <motion.li
      ...
      variants={animations.menuItemVariants}
      whileHover="hover"
      whileTap="tap"
    >
      ...
      <button
        ...
        style={{
          ...
          backgroundColor: isMenuOpen(menu.key)
            ? theme.expandedMenuBg
            : "transparent",
        }}
      >
        <span className="d-flex align-items-center" style={{ gap: "10px" }}>
          <i className={menu.icon} style={{ fontSize: "20px" }}></i>
          <span
            style={{
      ...
      <AnimatePresence>
        {isMenuOpen(menu.key) && (
          <motion.ul
            ...
            animate="open"
            exit="closed"
            variants={animations.subMenuVariants}
          >
            {menu.submenus
              .filter((submenu) => submenu.show === undefined || submenu.show)
              .map((submenu) => (
                <li key={submenu.path} className="submenu-item mb-1">
                  <SubmenuLink submenu={submenu} />
                </li>
              ))}
          </motion.ul>
        )}
  ...
  const SubmenuLink = ({ submenu }) => {
    const isActiveSubmenu = location.pathname === submenu.path;
    ...
    return (
      <Link
        ...
        style={{
          fontSize: "15px",
          fontWeight: "400",
          color: isActiveSubmenu ? theme.submenuActiveText : theme.submenuText,
          backgroundColor: isActiveSubmenu
            ? theme.submenuActiveBg
            : "transparent",
        ...
        onMouseLeave={(e) =>
          (e.currentTarget.style.backgroundColor = isActiveSubmenu
            ? theme.submenuActiveBg
            : "transparent")
        }
      >
  ...
  const SingleMenuItem = ({ menu }) => {
    const isActivePath = location.pathname === menu.path;
    ...
    return (
      <Link
        ...
        style={{
          color: theme.text,
          backgroundColor: isActivePath ? theme.activeItemBg : "transparent",
          fontSize: "17px",
          fontWeight: "500",
          padding: "10px 16px",
          borderRadius: "8px",
        ...
        onMouseEnter={(e) =>
          (e.currentTarget.style.backgroundColor = isActivePath
            ? theme.activeItemBg
            : theme.hoverBg)
        }
        onMouseLeave={(e) =>
