...
const GalleryAll = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!galleriesRes || galleriesRes.length === 0) {
      ...
      const galleriesWithPhotos = await Promise.all(
        galleriesRes.map(async (gallery) => {
          if (gallery.photo) {
            return gallery; // Jika photo sudah tersedia, langsung gunakan
          }
          try {
  ...
  const handleSave = (updatedGallery) => {
    if (currentGallery) {
      ...
      // Add new gallery
      setData((prevData) => [updatedGallery, ...prevData]);
    }
    setShowModal(false);
  };
