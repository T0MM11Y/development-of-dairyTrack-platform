...
const Pemesanan = () => {
  ...
  const calculateTotal = () => {
    ...
    form.order_items.forEach(item => {
      const product = availableProducts.find(p => p.product_type === parseInt(item.product_type));
      if (product) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...form,
      customer_name: firstName && lastName ? `${firstName} ${lastName}` : form.customer_name,
    };
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Pesanan Gagal',
        text: `Gagal membuat pesanan: ${err.message || 'Terjadi kesalahan pada sistem'}`,
  ...
  return (
    <>
      ...
      <div className="contact-container" style={{ opacity: submitting ? 0.7 : 1 }}>
