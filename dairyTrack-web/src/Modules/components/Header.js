...
const Header = () => {
  ...
  const toggleModal = async () => {
    ...
    if (token) {
      try {
        ...
        if (result.success) {
          history.push("/admin"); // Redirect to admin page if token is valid
          return;
        }
      } catch (error) {
    ...
    setShowModal(!showModal); // Show modal if token is invalid or not present
  ...
  // Periksa token hanya saat komponen dimuat
  useEffect(() => {
      ...
      if (token) {
        try {
          ...
          if (!result.success) {
            ...
            setShowModal(true); // Tampilkan modal login jika token tidak valid
          }
        } catch (error) {
          console.error("Error checking token:", error);
        }
    ...
    checkUserToken(); // Periksa token saat komponen dimuat
  }, []);
