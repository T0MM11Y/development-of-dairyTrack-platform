...
class ProductTypeController {
  ...
  Future<List<ProdukType>> getProductTypes() async {
    ...
    if (response['success']) {
      try {
        ...
        _logger.e('Error parsing product types: $e');
        throw Exception('Gagal mem-parsing jenis produk: $e');
      }
    } else {
      _logger.e('Failed to fetch product types: ${response['message']}');
      throw Exception(response['message']);
  ...
  Future<bool> createProductType({
    ...
    try {
      if (productName.isEmpty || price.isEmpty || unit.isEmpty) {
        _logger.e('Required fields are missing');
        throw Exception('Nama produk, harga, dan unit harus diisi');
      }
