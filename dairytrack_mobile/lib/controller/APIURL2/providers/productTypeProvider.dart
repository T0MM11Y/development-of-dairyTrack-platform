...
class ProductTypeProvider with ChangeNotifier {
  ...
  Future<bool> updateProductType(
    ...
    try {
      final userId = await AuthUtils.getUserId();
      final existingProduct = await _controller.getProductTypeById(id);
      final createdBy = existingProduct.createdBy?.id ?? 0;
      final success = await _controller.updateProductType(
        id: id,
        productName: productData['productName'] ?? '',
        productDescription: productData['productDescription'] ?? '',
  ...
  String _processErrorMessage(String error) {
    if (error.contains('A product with this name already exists')) {
      return 'A product with this name already exists';
    } else if (error.contains('Product name, price, and unit are required')) {
      return 'Product name, price, and unit are required';
