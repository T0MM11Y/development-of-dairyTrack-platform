...
class EditOrderModal extends StatefulWidget {
  ...
  const EditOrderModal({Key? key, required this.order}) : super(key: key);
...
class _EditOrderModalState extends State<EditOrderModal> {
  ...
  List<_OrderItemInput> _orderItems = [];
  ...
  void initState() {
    ...
    _locationController.text = widget.order.location;
    _notesController.text = widget.order.notes ?? '';
    _status = widget.order.status;
    _paymentMethod = widget.order.paymentMethod;
    _shippingCostController.text = widget.order.shippingCost ?? '';

    // Pisahkan kode negara dan nomor telepon
    String phone = widget.order.phoneNumber;
    for (var country in _countryCodes) {
      if (phone.startsWith(country['code']!)) {
        _countryCode = country['code']!;
        _phoneNumberController.text = phone.substring(country['code']!.length);
        break;
      }
    }
    if (_phoneNumberController.text.isEmpty) {
      _phoneNumberController.text = phone; // Jika tidak ada kode negara
    }

    _orderItems = widget.order.orderItems
        .map((item) => _OrderItemInput(
              productType: item.productType ?? item.productTypeDetail?.id,
  ...
  Future<void> _submitForm(OrderProvider provider) async {
    if (_formKey.currentState!.validate() &&
      ...
      try {
        ...
        _logger.i('Updating order: $orderData');

        final success = await provider.updateOrder(widget.order.id, orderData);
        if (success) {
  ...
  Widget _buildOrderItemInput(int index, _OrderItemInput item,
    ...
    ProdukType? currentProduct;
    if (item.productType != null) {
      try {
        currentProduct = productTypeProvider.productTypes
            .firstWhere((product) => product.id == item.productType);
      } catch (e) {
        _logger.w('Product with ID ${item.productType} not found');
        currentProduct = null;
      }
    }

    final dropdownItems = [
      if (currentProduct != null && !availableProducts.contains(currentProduct))
        DropdownMenuItem<int>(
          value: currentProduct.id,
          child: Text('${currentProduct.productName} (Tidak Tersedia)'),
        ),
      ...availableProducts.map((product) => DropdownMenuItem<int>(
            value: product.id,
    ...
    return Column(
      children: [
        ...
        else if (availableProducts.isEmpty && currentProduct == null)
          const Text(
...
class _OrderItemInput {
  ...
  _OrderItemInput({this.productType, String? quantity})
