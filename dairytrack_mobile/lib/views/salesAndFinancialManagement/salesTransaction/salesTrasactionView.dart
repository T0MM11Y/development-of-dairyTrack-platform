...
class SalesTransactionView extends StatefulWidget {
  const SalesTransactionView({super.key});

  @override
  State<SalesTransactionView> createState() => _SalesTransactionViewState();
}
...
class _SalesTransactionViewState extends State<SalesTransactionView>
  ...
  void _fetchInitialData() {
    final provider =
        Provider.of<SalesTransactionProvider>(context, listen: false);
    provider.fetchTransactions(queryString: _queryString, reset: true);
  }
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      if (!provider.isLoading && provider.hasMore) {
        provider.fetchTransactions(queryString: _queryString);
      }
    }
  }
        ...
        builder: (context, provider, child) {
          ...
          return RefreshIndicator(
            onRefresh: () => provider.fetchTransactions(
                queryString: _queryString, reset: true),
            child: SingleChildScrollView(
  ...
  Widget _buildSearchBar(SalesTransactionProvider provider) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: _searchController.text.isNotEmpty
            ? IconButton(
                ...
                onPressed: () {
                  _searchController.clear();
                  _queryString = _buildQueryString();
                  provider.setSearchQuery(null);
                  _fetchInitialData();
                },
              )
      ...
      onChanged: (value) {
        _queryString = _buildQueryString(searchQuery: value);
            ...
            children: [
              ElevatedButton(
                onPressed: () {
                  ...
                  final query = _buildQueryString();
                  _logger.i('Applying filter with query: $query');
                  final provider = Provider.of<SalesTransactionProvider>(
                      context,
                      listen: false);
                  provider
                      .fetchTransactions(queryString: query, reset: true)
                      .then((_) {
                    if (provider.errorMessage.isNotEmpty) {
                      _logger.e('Filter error: ${provider.errorMessage}');
                    } else {
                ...
                child:
                    const Text('Apply Filter', style: TextStyle(fontSize: 14)),
              ),
              if (_startDate != null ||
                  _endDate != null ||
                  _selectedPaymentMethod != null)
  ...
  String _buildQueryString({String? searchQuery}) {
    final params = <String, String>{};
    if (searchQuery != null && searchQuery.isNotEmpty) {
      params['search'] = Uri.encodeComponent(searchQuery);
    }
    if (_startDate != null) {
      params['start_date'] = DateFormat('yyyy-MM-dd').format(_startDate!);
    }
      ...
      children: paymentMethodMap.entries.toList().asMap().entries.map((entry) {
        ...
        return Row(
          ...
          children: [
            ...
            const SizedBox(width: 8),
            Text(
              '${item.key}: Rp ${item.value.toStringAsFixed(2)}',
              style: TextStyle(fontSize: 12, color: Colors.grey[700]),
            ),
          ],
        );
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Percentage of sales by product name',
              style: TextStyle(fontSize: 12, color: Colors.grey[600]),
            ),
            const SizedBox(height: 16),
            Container(
          ...
          children: [
            ...
            transactions.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      if (index == transactions.length && provider.hasMore) {
                        return const Center(
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Text(
                            'Order: ${item.order.orderNo}\nTotal: Rp ${item.totalPrice}\nMethod: ${item.paymentMethod}\nDate: ${DateFormat('dd MMM yyyy, HH:mm').format(item.transactionDate)}',
                            style: TextStyle(
                                fontSize: 12, color: Colors.grey[600]),
                          ),
                          trailing: Text(
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () {
                    Navigator.pop(context);
                    provider.exportToPdf(
                        queryString: _queryString, context: context);
                  },
                ),
                ListTile(
                  leading: Icon(Icons.table_chart, color: Colors.green[600]),
