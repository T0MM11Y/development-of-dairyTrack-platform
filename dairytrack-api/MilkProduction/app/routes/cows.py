...
def create_cow():
    ...
    # Retrieve farmer_id from the request data
    ...
    # Update the farmer's total_cattle count
    farmer.total_cattle = (farmer.total_cattle or 0) + 1

    # Commit the changes to the database
    db.session.commit()

    return jsonify({'message': 'Cow created successfully', 'data': cow.to_dict(), 'status': 201}), 201
...
def update_cow(id):
    ...
    db.session.commit()
    return jsonify({'message': 'Cow updated successfully', 'data': cow.to_dict(), 'status': 200}), 200
...
def delete_cow(id):
    ...
    return jsonify({'message': f'Cow with name "{cow_name}" and id "{id}" has been successfully deleted!', 'status': 200}), 200
