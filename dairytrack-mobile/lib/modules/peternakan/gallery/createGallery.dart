...
class CreateGallery extends StatefulWidget {
  const CreateGallery({Key? key}) : super(key: key);

  @override
  State<CreateGallery> createState() => _CreateGalleryState();
}
...
class _CreateGalleryState extends State<CreateGallery> {
  ...
  Future<void> _submitGallery() async {
    if (_formKey.currentState?.validate() != true || _photoFile == null) {
      _showErrorDialog('Please fill all fields and select a photo.');
      return;
    }
    ...
    try {
      final gallery = Gallery(
        photo: '', // Photo URL will be set by the API
        tittle: _titleController.text.trim(),
        createdAt: DateTime.now(),
        updatedAt: DateTime.now(),
      );

      await createGallery(gallery, _photoFile);

      _showSuccessDialog();
    } catch (e) {
      _showErrorDialog('Failed to create gallery: $e');
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  void _showSuccessDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Gallery created successfully.'),
        actions: [
          TextButton(
            onPressed: () {
