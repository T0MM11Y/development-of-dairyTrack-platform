...
class _FinanceListState extends State<FinanceList> {
  ...
  // Menghitung total amount per transaction_type
  Map<String, double> _calculateTransactionTypeTotals(List<Finance> finances) {
    final totals = {'income': 0.0, 'expense': 0.0};
    for (var finance in finances) {
      if (finance.transactionType == 'income') {
        totals['income'] = totals['income']! + finance.amount;
      } else if (finance.transactionType == 'expense') {
        totals['expense'] = totals['expense']! + finance.amount;
      }
    }
    return totals;
  }
            ...
            Expanded(
              child: FutureBuilder<List<Finance>>(
                ...
                builder: (context, snapshot) {
                  ...
                  final totals = _calculateTransactionTypeTotals(finances);
                                  ...
                                  PieChartData(
                                    sections: [
                                      PieChartSectionData(
                                        ...
                                        value: totals['income'],
                                        title:
                                            'Income\n${NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ', decimalDigits: 0).format(totals['income'])}',
                                        radius: 80,
                                      ...
                                      PieChartSectionData(
                                        ...
                                        title:
                                            'Expense\n${NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ', decimalDigits: 0).format(totals['expense'])}',
                                        radius: 80,
                                        titleStyle: const TextStyle(
                                          fontSize: 12,
                                          fontWeight: FontWeight.bold,
