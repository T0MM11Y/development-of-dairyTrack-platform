...
class _AllFeedStocksState extends State<AllFeedStocks> {
  ...
  Future<List<FeedStock>> fetchFeedStocks() async {
    try {
      final feedStocks = await getFeedStocks();
      ...
      // Instead of showing SnackBar here, throw the error to be handled by the caller
  ...
  Future<void> _exportToPDF() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Fitur ekspor PDF belum diimplementasikan')),
      );
    }
  }
  ...
  Future<void> _exportToExcel() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Fitur ekspor Excel belum diimplementasikan')),
      );
    }
  }

  void _refreshFeedStocks() {
    setState(() {
          ...
          Expanded(
            child: FutureBuilder<List<FeedStock>>(
              ...
              builder: (context, snapshot) {
                if (_feedStocksFuture == null) {
                  // Fallback in case _feedStocksFuture is null
                ...
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return const Center(
                      child: Text('Tidak ada stok pakan ditemukan.'));
                }
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Stok: ${feedStock.stock.toStringAsFixed(2)} kg',
                              style: TextStyle(color: Colors.grey[600]),
                            ),
                            Text(
