  ...
  Future<void> _handleSave() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Konfirmasi'),
        content: const Text(
            'Apakah Anda yakin ingin menyimpan perubahan pada data pakan harian?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Batal'),
  ...
  bool _validateFeedItems() {
    ...
    for (var item in _formList) {
      if (item['feed_id'] == null ||
          (item['quantity'] as String).trim().isEmpty) {
        _showErrorDialog('Form Tidak Lengkap',
      ...
      if (quantity <= 0) {
        _showErrorDialog(
            'Form Tidak Lengkap', 'Jumlah pakan harus lebih dari 0');
        return false;
      }
      final availableStock = _getFeedStock(item['feed_id'] as int?);
      if (quantity > availableStock) {
        final feedName =
            _feeds.firstWhere((feed) => feed.id == item['feed_id']).name;
        _showErrorDialog(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(_dailyFeed != null
                                  ? _dateFormat.format(_dailyFeed!.date)
                                  : '-'),
                            ],
                          ),
                        ),
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4.0),
                              Text(_dailyFeed != null
                                  ? _formatSession(_dailyFeed!.session)
                                  : '-'),
                            ],
                          ),
                        ),
                  ...
                  if (_isEditing) ...[
                    ..._formList.asMap().entries.map((entry) {
                      ...
                      return Column(
                        ...
                        children: [
                          ...
                          if (item['feed_id'] != null &&
                              (item['quantity'] as String).isNotEmpty)
                            Builder(builder: (context) {
                              final requestedQuantity =
                                  double.tryParse(item['quantity'] as String) ??
                                      0.0;
                              final availableStock =
                                  _getFeedStock(item['feed_id'] as int?);
                    ...
                    if (_feedItems.isEmpty)
                      Container(
                        ...
                        child:
                            const Text('Tidak ada data pakan untuk sesi ini.'),
                      )
                    else
                      Table(
